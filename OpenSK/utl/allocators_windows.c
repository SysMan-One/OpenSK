...
static void skPrintStackTrace(
  ...
  // TODO: Get this working for Windows.
  /*
...
static SkAllocationLocationIMPL* skGetDebugAllocationLocation(
  ...
  pLocation = skClearAllocate(
    pUserData->pAllocator,
    sizeof(SkAllocationLocationIMPL) + sizeof(PVOID) * stackDepth,
    1,
    SK_SYSTEM_ALLOCATION_SCOPE_LOADER
  );
  if (!pLocation) {
...
static void* SKAPI_CALL skAllocationFunction_Debug(
  ...
  capturedFames = CaptureStackBackTrace(0, pUserData->maxBacktraceCount, pUserData->pBacktraceArray, &traceHash);
  if (capturedFames < 0) {
    return NULL;
  }

  // Get the full backtrace (grab backtrace until we get all of it)
  while (capturedFames == pUserData->maxBacktraceCount) {
    ...
    pUserData->pBacktraceArray = skReallocate(
      ...
      sizeof(PVOID) * pUserData->maxBacktraceCount,
      1,
      SK_SYSTEM_ALLOCATION_SCOPE_LOADER
    );
